...
os.environ['TORCH_HOME'] = 'raid/ysharma_me/fair_lr/LfF'
...
def train(
    ...
    for lamda in [1e-4,1e-3,1e-5,1e-2]:
        ...
        writer = SummaryWriter(os.path.join(log_dir, f"summary_{lamda}", main_tag))
        ...
        for step in tqdm(range(main_num_steps)):
            ...
            logit_b_s = F.softmax(logit_b , dim=1)

            loss_kl = criterion(logit_d , logit_b.data.max(1, keepdim=True)[1].squeeze(1)) #CE
            ...
            loss = loss_d_update.mean() + (lamda * loss_kl).mean()
        ...
        os.makedirs(os.path.join(log_dir, f"result_{lamda}", main_tag), exist_ok=True)
        result_path = os.path.join(log_dir, f"result_{lamda}", main_tag, "result.th")
        model_path = os.path.join(log_dir, f"result_{lamda}", main_tag, "model.th")
